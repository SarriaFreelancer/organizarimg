...
export default function Home() {
      ...
      startTransition(async () => {
        try {
          ...
          if (recommendation && [2, 4, 6].includes(recommendation)) {
            ...
            toast({
              title: "AI Suggestion",
              description: `We suggest a ${recommended}-photo layout for your ${images.length} images.`,
            });
          }
        } catch (error) {
  ...
  const handleLoadSample = async () => {
    toast({ title: 'Loading sample images...', description: 'Please wait a moment.' });
  ...
  const HeroSection = () => (
    <div className="text-center flex flex-col items-center justify-center min-h-[calc(100vh-200px)] p-8">
      ...
      >
        <h1 className="font-headline text-5xl md:text-7xl font-bold tracking-tight mb-4">Photo Mosaic Generator</h1>
        <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
          Turn your memories into beautiful collages. Upload your photos, and let our AI suggest the perfect layout for a stunning result.
        </p>
        <div className="flex gap-4 justify-center">
          ...
          <Button size="lg" variant="secondary" onClick={handleLoadSample}>
            <Wand2 className="mr-2" /> Load Sample
          </Button>
        </div>
      </motion.div>
    </div>
  ...
  const MosaicCreator = () => (
    <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8 p-4 md:p-8">
      <aside className="lg:sticky top-8 self-start flex flex-col gap-6">
        <Card className="p-6">
          <h2 className="font-headline text-2xl font-semibold mb-4">Your Photos</h2>
          <div className="grid grid-cols-3 gap-2 mb-4 max-h-60 overflow-y-auto pr-2">
            {loadedImages.map((img, index) => (
              <div key={index} className="relative group aspect-square">
                <Image src={img.src} alt={`upload-preview-${index}`} width={100} height={100} className="rounded-md object-cover w-full h-full" />
          ...
          <Button variant="outline" className="w-full" onClick={() => fileInputRef.current?.click()}>
            <ImageIcon className="mr-2" /> Add More Photos
          </Button>
        </Card>
          ...
          <RadioGroup value={String(layout)} onValueChange={(val) => setLayout(Number(val) as LayoutOptions)} className="space-y-3">
            {[2, 4, 6].map(option => (
              <div key={option} className={`flex items-center space-x-3 p-3 rounded-lg transition-colors ${layout === option ? 'bg-accent' : ''}`}>
                ...
                <Label htmlFor={`layout-${option}`} className="flex-grow cursor-pointer">{option} Photos per Page</Label>
      ...
      <main>
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-headline text-3xl font-bold">Collage Preview</h2>
